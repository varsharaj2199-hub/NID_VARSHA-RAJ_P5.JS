...
let dialogueStep = 0;
let showPrompt = false; //to guide the player 



function preload() {
  spriteImage = loadImage('Images/1_2.png'); // sprite sheet of character 1
...
function setup() {
  ...
  // cutting the sprite sheet into frames
...
function draw() {
  ...
  // movement of character 1 when there is no dialogue and prompt
  ...
  if (scene === 5) {
    ...
    // when character 1 comes near character 2 , then dialogue begins
  ...
  if (showDialogue) {
    ...
    text("Character 2: Oh, what took you so long?.....Click to continue.", width / 2, 50);
      ...
      text("Character 1: Yes, let's watch!", width / 2, 50);
  ...
  if (showPrompt && !lanternActive) {
    ...
    textSize(22);
    textAlign(CENTER);
    text("Press SPACE to watch the lanterns", width / 2, 100);
  }
...
function mousePressed() {
  ...
  if (showIntro) {
    showIntro = false;
    x = width / 2 - 50; // character 1 starts bottom center
    return;
  }
